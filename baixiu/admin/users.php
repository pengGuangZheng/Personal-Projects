...
<body>
  ...
  <!-- <div class="aside">
    ...
    <script type="text/template" id="usersTpl"> 
             {{ each list }}
              <tr data-id="{{ $value.id }}">
                ...
                <td>{{$value.status}}</td>
                <td class="text-center">
                  <a href="#" class="btn btn-default btn-xs edit">编辑</a>
                  <a href="#" class="btn btn-danger btn-xs remove">删除</a>
                </td>
              </tr>
             {{ /each }}
    ...
    <script type="text/template" id="controlUserTpl">
      <h2> {{ id ? '修改用户':'添加新用户' }} </h2>

      <div class="config" 
           style="display:none;"
           data-url="{{ id ? './api/updateuser.php':'./api/insertuser.php'}}"
           data-success-text="{{ id ? '修改成功':'新增成功'}}"></div>
          {{if id }}
          ...
          {{ /if }}
            ...
            <div class="form-group">
              <button class="btn btn-primary" type="submit">{{id ?'编辑':'添加'}}</button>
            {{ if id }}
            <input class="btn btn-warning cancelbtn" type="button" value="取消编辑">
            {{ /if }}
            </div>
  ...
  <script>
    ...
    //使用一个空的对象渲染模板
    $('#controlUserFormId').html(template('controlUserTpl',{}));
   ...
   //绑定编辑事件的(代理)
  $(function(){
    $('#user_list_table tbody').on('click','.edit',function(){
      var updateId=$(this).closest('tr').attr('data-id');

      //利用id查询需要更新的数据

      $.get('./api/getuserbyid.php?id='+updateId,function(json){
        //拿到对象json.result[0]
          if(json.success){
            $('#controlUserFormId').html(template('controlUserTpl',json.result[0]));
    ...
    $(function(){
      ...
      $('#controlUserFormId').submit(function(){
        ...
        //成功,则发送ajax
        var url = $( '#controlUserFormId .config' ).attr( 'data-url' );
        var msg = $( '#controlUserFormId .config' ).attr( 'data-success-text' );
        
        $.post( url, formdata, function ( json ) {
          if ( json.success ) {
            // 提示
            $( '#msg' ).fadeIn( 200 ).find( 'span' ).text( msg );
            setTimeout( function () {
              $( '#msg' ).fadeOut( 500 );
            }, 3000);
    ...
    //取消编辑事件
    $( function () {
      $( '#controlUserFormId' ).on( 'click', '.cancelbtn', function () {
    ...
    //别名的input事件
    $(function(){
      $('#controlUserFormId' ).on('input','#slug',function(){
